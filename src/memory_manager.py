...
class ConversationMemory:
        ...
        try:
            ...
            for topic, weight in all_topics.items():
                if topic in self.topic_weights:
                    self.topic_weights[topic] = (self.topic_weights[topic] + weight) / 2
                else:
            ...
            self.conversation_embeddings.append(current_semantic_hash)
            new_topics = set(all_topics.keys())
            topic_similarity = self._calculate_topic_similarity(self.current_topics, new_topics)
            ...
            self.logger.info(f"Added interaction to memory. Topics: {list(new_topics)[:5]}...")
            
        except Exception as e:
            self.logger.error(f"Failed to add interaction to memory: {str(e)}") 
    ...
    def _classify_topic_transition(self, old_topics: Set[str], new_topics: Set[str]) -> str:
        ...
        overlap_ratio = len(old_topics.intersection(new_topics)) / len(old_topics.union(new_topics))
    ...
    def _calculate_interaction_relevance(self, current_query: str, current_topics: Dict[str, float], 
        ...
        try:
            interaction_topics = interaction["query_topics"].union(interaction["response_topics"])
            topic_similarity = self._calculate_topic_similarity(set(current_topics.keys()), interaction_topics)
            interaction_hash = interaction.get("semantic_hash", "")
            semantic_similarity = (
                0.5 if interaction_hash == current_semantic_hash and interaction_hash else
                self._calculate_semantic_similarity(current_query, interaction["query"])
            )
            recency_score = self._calculate_recency_score(interaction["timestamp"])
            source_overlap = self._calculate_source_overlap(current_query, interaction)
    ...
    def _build_smart_context(self, relevant_interactions: List[Dict[str, Any]], max_length: int) -> str:
        try:
            ...
            for item in relevant_interactions:
                ...
                response_preview_length = min(300 if relevance > 0.5 else 150, len(interaction["response"]))
                ...
                if relevance > 0.4 and interaction["sources"]:
                    source_files = [s.get("filename", "unknown") for s in interaction["sources"][:2]]
                ...
                if current_length + len(context_part) <= max_length:
                    ...
                    if len(context_parts) == 0:
                        context_parts.append(context_part[:max_length - 50] + "...\n")
                    break
    ...
    def _summarize_context(self, context: str, max_length: int) -> str:
        try:
            ...
            if len(summarized) > max_length:
                summarized = summarized[:max_length - 50] + "\n[Контекст сокращен...]\n"
        ...
        try:
            ...
            if len(self.topic_transitions) > 1:
                ...
                for topic, count in common_topics:
                    if count > 1:
                        suggestions.append(f"Какие еще аспекты {topic} важны?")
    ...
    def _find_advanced_question_patterns(self) -> List[str]:
        try:
            if len(self.conversation_history) < 2:
                return []
            recent_queries = [interaction["query"] for interaction in list(self.conversation_history)[-5:]]
            pattern_suggestions = {
            ...
            self.logger.warning(f"Advanced pattern analysis failed: {str(e)}")
            return []
    ...
    def _calculate_topic_coherence(self) -> float:
        ...
        for i in range(len(self.topic_transitions) - 1):
            ...
            similarity = self._calculate_topic_similarity(current_topics, next_topics)
    ...
    def get_conversation_context(self, current_query: str, max_context_length: int = 1500) -> str:
        try:
            if not self.conversation_history:
                return ""
            cache_key = hashlib.md5(f"{current_query}_{max_context_length}".encode()).hexdigest()
            ...
            current_semantic_hash = self._generate_semantic_hash(current_query, "")
            
            relevant_interactions = []
            recent_interactions = list(self.conversation_history)[-min(self.context_window * 2, len(self.conversation_history)):]
            ...
            relevant_interactions.sort(key=lambda x: x["relevance"], reverse=True)
            context = self._build_smart_context(relevant_interactions, max_context_length)
            self._context_cache[cache_key] = context
            
            if context:
                self.stats["context_used_count"] += 1
                self.stats["average_context_length"] = (
                    (self.stats["average_context_length"] * (self.stats["context_used_count"] - 1) + len(context))
                    / self.stats["context_used_count"]
                )
            
            self.logger.info(f"Generated enhanced context with {len(relevant_interactions)} interactions, {len(context)} chars")
            return context
            
        except Exception as e:
            self.logger.error(f"Failed to generate conversation context: {str(e)}")
            return ""
    
    def get_follow_up_suggestions(self, current_query: str, response: str) -> List[str]:
        try:
            suggestions = []
            current_topics = self._extract_topics_with_weights(current_query + " " + response)
            entities = self._extract_entities_enhanced(response)
            sorted_topics = sorted(current_topics.items(), key=lambda x: x[1], reverse=True)
            ...
            flow_suggestions = self._analyze_conversation_flow(current_query, response)
            ...
            ranked_suggestions = self._rank_and_deduplicate_suggestions(suggestions, current_topics)
            return ranked_suggestions[:6]
            
        except Exception as e:
            self.logger.error(f"Failed to generate follow-up suggestions: {str(e)}")
            return []
    ...
    def detect_conversation_shift(self, current_query: str) -> Dict[str, Any]:
        try:
            ...
            if shift_detected:
                self.logger.info(f"Conversation shift detected with confidence {shift_confidence:.2f}")
            ...
            self.logger.error(f"Failed to detect conversation shift: {str(e)}")
    ...
    def _extract_topics_with_weights(self, text: str) -> Dict[str, float]:
        try:
            ...
            noun_phrases = re.findall(r'\b[А-Яа-я]{4,}\s+[А-Яа-я]{4,}(?:\s+[А-Яа-я]{4,})?\b', text)
            for phrase in noun_phrases:
                clean_phrase = phrase.lower().strip()
                if clean_phrase and len(clean_phrase) > 6:
                    topics_with_weights[clean_phrase] = min(0.8 + (len(clean_phrase.split()) * 0.1), 1.0)
            domain_patterns = [
            ...
            for pattern in domain_patterns:
                ...
                for match in matches:
                    if match not in stop_words:
                        topics_with_weights[match] = min(topics_with_weights.get(match, 0) + 0.3, 1.0)
            self._topic_cache[cache_key] = topics_with_weights
            return topics_with_weights
            
        except Exception as e:
            self.logger.error(f"Topic extraction with weights failed: {str(e)}")
            return {}
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        total_cache_requests = self.advanced_stats["cache_hits"] + self.advanced_stats["cache_misses"]
        cache_hit_rate = (
