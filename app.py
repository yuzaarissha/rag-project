...
if page == "Главная":
    ...
    with col2:
        ...
        - **Обработка документов:** PyMuPDF, python-docx
        - **Интерфейс:** Streamlit
        """)
    ...
    with col2:
        ...
        st.markdown("""
        1. Перейдите в **"Документы"** и загрузите файлы (PDF, DOCX, TXT)
        ...
        with tab1:
            ...
            if upload_method == "Загрузить файлы":
                ...
                if os.path.exists(docs_dir):
                    ...
                    if supported_files:
                        st.write(f"**Найдено {len(supported_files)} файлов:**")
                        ...
                        with st.expander(expander_title, expanded=False):
                            ...
                            with col_info:
                                ...
                                if file_ext == '.pdf' and page_count != 'N/A':
                                    ...
                                    st.write(
                                        "• Страницы: отсутствуют (непрерывный текст)")
    ...
    if not st.session_state.system_initialized:
        ...
        with tab1:
            ...
            with col1:
                ...
                st.markdown(
                    "- **deepseek-r1** — reasoning-модель от DeepSeek (1.5B, 7B, 8B, 14B, 32B, 70B, 671B)")
                st.markdown(
                    "- **qwen3** — новая генерация LLM от Alibaba (0.6B–235B)")
                st.markdown(
                    "- **llava** — мультимодальная модель от Vicuna + CLIP (7B, 13B, 34B)")
                st.markdown(
                    "- **qwen2.5vl** — vision-language модель от Qwen (3B, 7B, 32B, 72B)")
            with col2:
                st.markdown("#####")
                st.markdown("- **llama3.2** — компактная LLM от Meta (1B, 3B)")
                st.markdown("- **phi3** — лёгкая LLM от Microsoft (3.8B, 14B)")
                st.markdown(
                    "- **gemma3** — мощная модель Gemma на одном GPU (1B, 4B, 12B, 27B)")
                st.markdown(
                    "- **codellama** — модель генерации и обсуждения кода (7B, 13B, 34B, 70B)")
                st.markdown(
                    "- **mistral** — компактная LLM от Mistral AI (7B)")
                st.markdown("- **gemma3n** — модель для устройств (e2b, e4b)")
            with col3:
                st.markdown("##### Embedding модели")
                st.markdown(
                    "- **nomic-embed-text** — универсальная embedding-модель от Nomic (33.8M)")
                st.markdown(
                    "- **mxbai-embed-large** — эмбеддинг от Mixedbread.ai (335M)")
                st.markdown(
                    "- **bge-m3** — многоязычная embedding-модель от BAAI (567M)")
                st.markdown(
                    "- **all-minilm** — компактные эмбеддинги от Microsoft/CoSENT (22M, 33M)")
                st.markdown(
                    "- **snowflake-arctic-embed** — эмбеддинги от Snowflake (22M, 33M, 110M, 137M, 335M)")
            ...
            st.caption(
                "Низкие значения: система отвечает чаще, но может ошибаться")
            st.caption("Высокие значения: система отвечает реже, но точнее")
            if new_threshold != current_threshold:
                st.session_state.rag_pipeline.router.update_confidence_threshold(
                    new_threshold)
                st.success(
                    f"Настройки маршрутизатора обновлены: порог уверенности {new_threshold}")
            ...
            if chunk_size != st.session_state.chunk_size or chunk_overlap != st.session_state.chunk_overlap:
                ...
                st.success(
                    f"Настройки обработки обновлены: размер фрагмента {chunk_size}, перекрытие {chunk_overlap}")
            ...
            if search_k != st.session_state.search_k:
                ...
                st.success(
                    f"Настройки поиска обновлены: количество результатов {search_k}")
            st.caption(
            ...
            if temperature != st.session_state.temperature:
                ...
                st.success(
                    f"Настройки LLM обновлены: температура генерации {temperature}")
            ...
            if st.session_state.conversation_history:
                ...
                if st.button("Экспортировать историю разговоров"):
                    ...
                    st.download_button(
                        ...
                        data=export_text,
                        file_name=f"history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                        mime="text/plain"
                    )
            else:
            ...
            with col1:
                ...
                if available_models['llm']:
                    ...
                    if len(available_models['llm']) > 5:
                        st.write(
                            f"  ... и еще {len(available_models['llm']) - 5}")
            with col2:
                st.metric("Embedding модели", len(
                    available_models['embedding']))
                if available_models['embedding']:
                    st.write("Список:")
                    ...
                    if len(available_models['embedding']) > 5:
                        st.write(
                            f"  ... и еще {len(available_models['embedding']) - 5}")
